...
std::vector<std::pair<double, double>> Polytopes::getBounds() const {
    ...
    std::vector<std::pair<double, double>> bounds(dimensions, {0.0, 0.0});

    // Compute loose bounds for each dimension
        ...
        for (size_t i = 0; i < A.size(); ++i) {
            if (A[i][dim] != 0) {
                ...
                if (A[i][dim] > 0) {
                    ...
                    minBound = std::min(minBound, bound);
                }
            }
        }

        bounds[dim] = {minBound, maxBound};
    }
